...
class DefaultBrowserPromptsExperimentImpl @Inject constructor(
    ...
    /**
     * Caches deserialized [Toggle.getSettings] for [DefaultBrowserPromptsFeatureToggles.additionalPrompts].
     *
    ...
    private suspend fun evaluate() {
        val isEnrolled = defaultBrowserPromptsFeatureToggles.additionalPrompts().getCohort() != null
    ...
    private suspend fun DefaultBrowserPromptsFeatureToggles.parseFeatureSettings(): FeatureSettings? = withContext(dispatchers.io()) {
        return@withContext try {
            featureSettingsJsonAdapter.fromJson(additionalPrompts().getSettings()!!)
        } catch (e: Exception) {
            Timber.e(e)
            null
        }
    ...
    private fun DefaultBrowserPromptsFeatureToggles.getEnrollmentDate(): Date? =
        additionalPrompts().getCohort()?.enrollmentDateET?.let { enrollmentZonedDateET ->
    ...
    private fun DefaultBrowserPromptsFeatureToggles.getOrAssignCohort(): AdditionalPromptsCohortName? {
        for (cohort in AdditionalPromptsCohortName.entries) {
            if (additionalPrompts().isEnabled(cohort)) {
                return cohort
            }
        }
        return null
