...
class AutofillStoredBackJavascriptInterface @Inject constructor(
    ...
    private suspend fun handleRequestForPartialSave(
        ...
        if (!autofillFeature.partialFormSaves().isEnabled()) {
            Timber.w("BrowserAutofill: storeFormData called with partial save but feature is disabled")
            return
        }
    ...
    private suspend fun isBackFillingUsernameSupported(
        ...
        if (!autofillFeature.partialFormSaves().isEnabled()) {
            Timber.w("Will not attempt username backfilling as partial saves feature is disabled")
