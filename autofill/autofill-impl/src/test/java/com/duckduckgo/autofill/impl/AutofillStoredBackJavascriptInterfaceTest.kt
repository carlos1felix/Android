...
class AutofillStoredBackJavascriptInterfaceTest {
    ...
    fun setUp() = runTest {
        ...
        autofillFeature.partialFormSaves().setRawStoredState(Toggle.State(enable = true))
    ...
    @Test
    fun whenPartialFormSubmissionWhenFeatureDisabledThenNoActionTaken() = runTest {
        autofillFeature.partialFormSaves().setRawStoredState(Toggle.State(enable = false))
        configureRequestParserToReturnPartialSaveRequestType(username = "user")
        whenever(currentUrlProvider.currentUrl(anyOrNull())).thenReturn("https://example.com")
        testee.storeFormData("")
        verify(partialCredentialSaveStore, never()).saveUsername(url = any(), username = any())
    }
    ...
    @Test
    fun whenFormSubmissionCouldBeBackfilledButFeatureFlagDisabledThenUsernameNotBackfilled() = runTest {
        whenever(partialCredentialSaveStore.getUsernameForBackFilling(any())).thenReturn("username-from-backfill")
        configureRequestParserToReturnSaveCredentialRequestType(username = null, password = "password")
        autofillFeature.partialFormSaves().setRawStoredState(Toggle.State(enable = false))
        testee.storeFormData("")
        assertNull(testCallback.credentialsToSave!!.username)
    }
    ...
    private suspend fun configureRequestParserToReturnSaveCredentialRequestType(
        ...
        whenever(passwordEventResolver.decideActions(anyOrNull(), any(), eq(NoMatch))).thenReturn(listOf(Actions.PromptToSave))
    }
